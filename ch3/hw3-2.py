...
def get_movies(dom):
    ...
    for row in rows:
        ...
        movie['intro'] = get_complete_intro(row.find('div', 'release_movie_name').a['href'])
...
def get_complete_intro(url):
    page = get_web_page(url)

    if page:
        soup = BeautifulSoup(page, 'html5lib')
        intro = soup.find('div', 'gray_infobox_inner').span.stripped_strings
        return list(intro)
